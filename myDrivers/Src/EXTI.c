...
void EXTI_Init(EXTI_InitTypeDef_t *EXTI_InitStruct)
{
	...
	EXTI->IMR &= ~(0x1U << EXTI_InitStruct->EXTI_LineNumber); //temizleme
	EXTI->EMR &= ~(0x1U << EXTI_InitStruct->EXTI_LineNumber); //temizleme
	...
	if(EXTI_InitStruct->EXTI_LineCmd != DISABLE)
	{
		...
		*((__IO uint32_t*)tempValue) = (0x1U << EXTI_InitStruct->EXTI_LineNumber);
		...
		EXTI->RTSR &= ~(0x1 << EXTI_InitStruct->EXTI_LineNumber);
		EXTI->FTSR &= ~(0x1 << EXTI_InitStruct->EXTI_LineNumber);
